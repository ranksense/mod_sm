 ...
 *
 * RanksenseIPHeader X-SM-CONNECTING-IP
...
module AP_MODULE_DECLARE_DATA ranksense_module;

#define CF_DEFAULT_IP_HEADER "X-SM-CONNECTING-IP"
...
static void *create_ranksense_server_config(apr_pool_t *p, server_rec *s)
{
    ranksense_config_t *config = apr_pcalloc(p, sizeof *config);
...
static void *merge_ranksense_server_config(apr_pool_t *p, void *globalv,
                                            void *serverv)
{
    ranksense_config_t *global = (ranksense_config_t *) globalv;
    ranksense_config_t *server = (ranksense_config_t *) serverv;
    ranksense_config_t *config;

    config = (ranksense_config_t *) apr_palloc(p, sizeof(*config));
...
static const char *secret_set(cmd_parms *cmd, void *dummy,
                                   const char *arg)
{
    ranksense_config_t *config = ap_get_module_config(cmd->server->module_config,
                                                       &ranksense_module);
...
static int ranksense_modify_connection(request_rec *r)
{
    conn_rec *c = r->connection;
    ranksense_config_t *config = (ranksense_config_t *)
        ap_get_module_config(r->server->module_config, &ranksense_module);

    ranksense_conn_t *conn;
    ...
    char *secret = (char *) apr_table_get(r->headers_in, "X-SM-SECRET");
    char *proxy_ips = NULL;
    char *parse_remote;
    char *eos;
    ...
    if (config->secret) {
        ...
        secret_len = strlen(config->secret);
        if (secret_len != strnlen(secret, secret_len+1))
            return 403;

        if (strncmp(secret, config->secret, secret_len))
            return 403;
    } else {
        return OK;
    }

    apr_pool_userdata_get((void*)&conn, "mod_ranksense-conn", c->pool);
    ...
    /* Deny requests that do not have a RanksenseRemoteIPHeader set when
     * DenyAllButRanksense is set. Do not modify the request otherwise and
     * return early.
     */
    if (!remote) {
        return OK;
...
#if AP_MODULE_MAGIC_AT_LEAST(20111130,0)
        if (!conn) {
            conn = (ranksense_conn_t *) apr_palloc(c->pool, sizeof(*conn));
            apr_pool_userdata_set(conn, "mod_ranksense-conn", NULL, c->pool);
...
ditto_request_rec:
    ...
    if (conn->proxy_ips) {
        apr_table_setn(r->notes, "ranksense-proxy-ip-list", conn->proxy_ips);
...
static const command_rec ranksense_cmds[] =
{
    AP_INIT_TAKE1("RanksenseRemoteIPHeader", header_name_set, NULL, RSRC_CONF,
...
static void register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_post_read_request(ranksense_modify_connection, NULL, NULL, APR_HOOK_REALLY_FIRST - 10);
}
...
module AP_MODULE_DECLARE_DATA ranksense_module = {
    ...
    create_ranksense_server_config, /* create per-server config structure */
    merge_ranksense_server_config,  /* merge per-server config structures */
    ranksense_cmds,                 /* command apr_table_t */
